...
public class EtattransportBean implements Serializable {
	...
	public String getetatclient2() {
		...
		for (Bonlivraison bl : listLivraison) {
			if (clients != null && !clients.isEmpty() && bl.getClient().getNom().equals(clients))
	...
	public String getetatLivraisons1() {
		...
		for (Bonlivraison bl : listbl) {
			if (matricule != null && !matricule.isEmpty() && bl.getVhecule().getMatricule().equals(matricule))
				listbl3.add(bl);
			else if (matricule == null || matricule.isEmpty())
				listbl3.add(bl);
		...
		for (Bonlivraison bl : listbl3) {
			if (chauffeur != null && !chauffeur.isEmpty() && bl.getChauffeur().getNompenom().equals(chauffeur))
				listBonlivraison.add(bl);

			else if (chauffeur == null || chauffeur.isEmpty())
				listBonlivraison.add(bl);
