...
@ManagedBean(name = "FacturefacBean")
@SessionScoped
...
public void setDates(String dates) {
	...
	public String savefacture() {
		...
		} else if (f2.getDate().getYear() == facturepassager.getDate().getYear() && f2 != null) {
		...
		Convert C = new Convert();
		facturepassager.setDate(new Date());
		facturepassager.setTimbres(p.getTimbre());
		facturepassager.setTotaltva(totaltva);
		facturepassager.setTotalht(mh);
		facturepassager.setTotalttc(
	...
	public String facturer() {
		...
		} else if (f2.getDate().getYear() == facture.getDate().getYear() && f2 != null) {
	...
	public String getfacturebyClientandtypepayement() {
		...
		listfacture = new ArrayList<Facture>();
		lbyfacture=servicefacture.getfacturebetwenndate(date, date2);
		...
		if (client!=null) {
			...
			for (Facture f : lbyfacture) {
				if (f.getBl().getClient().getNom().equals(client.getNom()))
					listfacture.add(f);
